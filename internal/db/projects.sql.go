...
const getProjectDefaultBranch = `-- name: GetProjectDefaultBranch :one
SELECT default_branch FROM projects WHERE id = ?
`

func (q *Queries) GetProjectDefaultBranch(ctx context.Context, id int64) (string, error) {
	row := q.db.QueryRowContext(ctx, getProjectDefaultBranch, id)
