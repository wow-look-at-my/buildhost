...
// apePrintfELF matches the printf call the trampoline redirects into its own
// copy to write an architecture's ELF header. The argument is a single-quoted
// shell word, so backslash is its only escape.
