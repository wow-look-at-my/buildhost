...
// apePrintfELF matches a `printf '...' >&7` call, which the trampoline uses to
// write an architecture's ELF header over its own copy. The argument is a
// single-quoted shell word, so backslash is its only escape.
