...
App.renderRegistryContent = function (bu, svc, project, isPrivate, method) {
    var p = App.h(project);
    ...
    if (method === "dl") {
        ...
        html += "<tr><td class='info-label'>Latest</td><td class='endpoint-cell'>" + App.urlTpl(dl + "/" + p + "?os={os}&arch={arch}", dl + "/" + p + "?os=", "&arch=") + "</td></tr>";
        html += "<tr><td class='info-label'>Version</td><td class='endpoint-cell'><code>" + App.h(dl + "/" + p + "?v={version}&os={os}&arch={arch}") + "</code><copy-btn data-src='code'></copy-btn></td></tr>";
        html += "<tr><td class='info-label'>Branch</td><td class='endpoint-cell'><code>" + App.h(dl + "/" + p + "?branch={branch}&os={os}&arch={arch}") + "</code><copy-btn data-src='code'></copy-btn></td></tr>";
        html += "</table>";
        if (isPrivate) {
        ...
        html += '<table class="info-table">';
        html += "<tr><td class='info-label'>Release</td><td class='endpoint-cell'><a href='" + App.h(apt + "/" + p + "/dists/stable/Release") + "'>" + App.h(apt + "/" + p + "/dists/stable/Release") + "</a><copy-btn data-src='a'></copy-btn></td></tr>";
        html += "<tr><td class='info-label'>InRelease</td><td class='endpoint-cell'><a href='" + App.h(apt + "/" + p + "/dists/stable/InRelease") + "'>" + App.h(apt + "/" + p + "/dists/stable/InRelease") + "</a><copy-btn data-src='a'></copy-btn></td></tr>";
        html += "<tr><td class='info-label'>Packages</td><td class='endpoint-cell'><code>" + App.h(apt + "/" + p + "/dists/stable/main/binary-{arch}/Packages") + "</code><copy-btn data-src='code'></copy-btn></td></tr>";
        html += "<tr><td class='info-label'>Pool</td><td class='endpoint-cell'><code>" + App.h(apt + "/" + p + "/pool/{filename}") + "</code><copy-btn data-src='code'></copy-btn></td></tr>";
        html += "<tr><td class='info-label'>Public Key</td><td class='endpoint-cell'><a href='" + App.h(apt + "/" + p + "/key.asc") + "'>" + App.h(apt + "/" + p + "/key.asc") + "</a><copy-btn data-src='a'></copy-btn></td></tr>";
        html += "</table>";
        if (isPrivate) {
            html += App.codeBlock("Setup", 'sudo install -d -m 0755 /etc/apt/keyrings\n# the token is the HTTP Basic password (username is ignored)\ncurl -fsSL -u "token:$TOKEN" ' + apt + '/' + project + '/key.asc | sudo gpg --dearmor -o /etc/apt/keyrings/buildhost.gpg\necho "deb [signed-by=/etc/apt/keyrings/buildhost.gpg] ' + apt + '/' + project + ' stable main" \\\n  | sudo tee /etc/apt/sources.list.d/' + project + '.list\n# both apt (metadata) and static (the .deb download redirect) need the token\ncat <<EOF | sudo tee /etc/apt/auth.conf.d/buildhost.conf\nmachine ' + aptHost + ' login token password $TOKEN\nmachine ' + staticHost + ' login token password $TOKEN\nEOF\nsudo chmod 600 /etc/apt/auth.conf.d/buildhost.conf\nsudo apt update && sudo apt install ' + project);
        } else {
        ...
        html += '<tr><td class="info-label">Formula</td><td class="endpoint-cell"><a href="' + App.h(brew + "/Formula/" + p + ".rb") + '">' + App.h(brew + "/Formula/" + p + ".rb") + "</a><copy-btn data-src='a'></copy-btn></td></tr></table>";
        if (isPrivate) {
            html += App.codeBlock("Install", "HOMEBREW_BUILDHOST_TOKEN=$TOKEN brew tap pazer/build " + brew + "/tap.git\nHOMEBREW_BUILDHOST_TOKEN=$TOKEN brew install pazer/build/" + project);
        } else {
        ...
        html += '<table class="info-table">';
        html += "<tr><td class='info-label'>Package metadata</td><td class='endpoint-cell'><a href='" + App.h(npm + "/@buildhost/" + p) + "'>" + App.h(npm + "/@buildhost/" + p) + "</a><copy-btn data-src='a'></copy-btn></td></tr>";
        html += "<tr><td class='info-label'>Tarball</td><td class='endpoint-cell'><code>" + App.h(npm + "/@buildhost/" + p + "/-/" + p + "-{version}.tgz") + "</code><copy-btn data-src='code'></copy-btn></td></tr>";
        html += "</table>";
        if (isPrivate) {
        ...
        html += "<tr><td class='info-label'>Manifest</td><td class='endpoint-cell'><code>" + App.h(oci + "/v2/" + p + "/manifests/{reference}") + "</code><copy-btn data-src='code'></copy-btn></td></tr>";
        html += "<tr><td class='info-label'>Blob</td><td class='endpoint-cell'><code>" + App.h(oci + "/v2/" + p + "/blobs/{digest}") + "</code><copy-btn data-src='code'></copy-btn></td></tr>";
        html += "</table>";
        if (isPrivate) {
            html += App.codeBlock("Docker pull", "echo $TOKEN | docker login " + ociHost + " -u token --password-stdin\ndocker pull " + ociHost + "/" + project + ":latest");
        } else {
            html += App.codeBlock("Docker pull", "docker pull " + ociHost + "/" + project + ":latest");
        }
        html += "</div>";
        ...
        html += '<table class="info-table">';
        html += "<tr><td class='info-label'>Deploy</td><td class='endpoint-cell'><code>PUT " + App.h(sites + "/" + p + "/branch/{branch}") + "</code><copy-btn data-src='code'></copy-btn></td></tr>";
        html += "<tr><td class='info-label'>Serve</td><td class='endpoint-cell'><a href='" + App.h(sites + "/" + p + "/") + "'>" + App.h(sites + "/" + p + "/") + "</a><copy-btn data-src='a'></copy-btn></td></tr>";
        html += "<tr><td class='info-label'>Delete</td><td class='endpoint-cell'><code>DELETE " + App.h(sites + "/" + p + "/branch/{branch}") + "</code><copy-btn data-src='code'></copy-btn></td></tr>";
        html += "<tr><td class='info-label'>List branches</td><td class='endpoint-cell'><a href='" + App.h(sites + "/" + p + "/branches") + "'>" + App.h(sites + "/" + p + "/branches") + "</a><copy-btn data-src='a'></copy-btn></td></tr>";
        html += "</table>";
        ...
        html += "<tr><td class='info-label'>Publish</td><td class='endpoint-cell'><code>POST " + App.h(bu + "/api/v1/projects/" + p + "/releases/{version}/publish") + "</code><copy-btn data-src='code'></copy-btn></td></tr>";
        html += "</table>";
...
App.pages.registries = function () {
    ...
    App.fetch("/registries").then(function (d) {
        ...
        function render() {
            ...
            var out = App.renderRegistryContent(bu, svc, savedProject, proj.is_private, savedMethod);
