class CopyBtn extends HTMLElement {
    ...
    get copyText(): string {
        ...
        if (src) {
            const scope: Element | Document = this.closest("td") || this.closest(".code-block") || this.closest(".card") || document;
            const el = scope.querySelector(src);
            if (!el) return "";
            return el.getAttribute("data-copy") || el.textContent || "";
        }

        let tpl: Element | null = this.closest(".url-tpl");
        if (!tpl) {
            const scope: Element | Document = this.closest("td") || this.closest(".card") || document;
            tpl = scope.querySelector(".url-tpl");
        }
        if (tpl) {
            let url = tpl.getAttribute("data-tpl") || "";
            tpl.querySelectorAll<HTMLSelectElement>(".tpl-select").forEach((sel) => {
        ...
        for (let i = 0; i < childNodes.length; i++) {
            ...
            else if ((node as Element).tagName !== "BUTTON") text += node.textContent;
        }
        return text.trim();
    }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll(".url-tpl").forEach((tplEl) => {
        tplEl.querySelectorAll<HTMLSelectElement>('.tpl-select[data-var="os"]').forEach((sel) => {
            sel.value = defaultOS;
        });
        tplEl.querySelectorAll<HTMLSelectElement>('.tpl-select[data-var="arch"]').forEach((sel) => {
