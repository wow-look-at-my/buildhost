...
#
# So the image puts a shebang launcher on PATH and keeps the APE beside it, and
# every spelling has to land on that launcher.
#
...
shared:
	files:
		export.sh: |
			# Flatten the image a single time. dats runs tests concurrently,
			# so a test that consumed another test's output would race it.
			set -eu
			...
			# through a shell, so a bare exec of a single succeeds and these
			# tests stop being able to fail. The handler is host-wide and
			# containers inherit it, so refuse to report a green nobody earned.
