...
- **`buildhost-publish-site`**: it records the uploaded tar.gz or zip archive's sha256. It carries **no** `artifact_url`. A site is served unpacked, so no URL returns those bytes, and a record must never point at bytes that hash to something else. This runs on every PR-preview push. The reusable `buildhost-preview.yml` and buildhost's own `preview.yml` therefore both declare `artifact-metadata: write`.
