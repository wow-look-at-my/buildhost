...
`GetCompressed` (the optional `CompressedGetter` capability, implemented by `Filesystem` and forwarded by `TracedStorage`) returns the stored bytes **without** decompressing -- the raw zstd stream for a compressed blob (Encoding `zstd`), or the identity bytes for one stored raw -- so a handler can pass `Content-Encoding: zstd` straight through to a zstd-accepting client and skip server-side decompression entirely.
