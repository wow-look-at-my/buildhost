...
`PATCH /api/v1/projects/{project}` (`UpdateProjectSettings`) updates the operator-set project settings. It takes `create_service`. A body that names `versioning` gets a 403 and changes nothing. A write token includes a CI job's OIDC token, so versioning changes only from the admin dashboard's project page. A switch to `auto` there keeps the existing releases, and the next number follows the highest one. Request fields are pointer-typed. An absent one is unchanged. `parseRoute` counts PATCH as a write verb. The centralized requireProject therefore demands a write-scoped token authorized for the project. The release-create body accepts the same optional `create_service` bool, asserted on every publish, and an absent one is untouched. The publishing repo's CI is therefore the declarative surface. No documented flow has a manual API step.
