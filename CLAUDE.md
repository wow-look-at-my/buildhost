...
- Apex `latest` (a download with no `?v=` and no `?branch=` -- e.g. `dl/{project}/latest/{os}/{arch}`) resolves to the newest published release on **master**, the assumed default branch (`LatestBranch` in `internal/db/releases.go`), so a push to a feature branch never hijacks `latest`. When master has no published release yet, unqualified `latest` is not available. Centralized in `db.GetLatestRelease`, so dl/brew/apt/web, the OCI `latest` tag, and the npm `latest` dist-tag stay consistent. Per-branch downloads (`?branch=`) are unaffected.
...
- **Browser sign-in with GitHub**: a browser hitting a private resource (e.g. a private static-site PR preview or download) used to get a raw JSON `401` with no way to authenticate. When GitHub login is configured (`BUILDHOST_GITHUB_CLIENT_ID` + `BUILDHOST_GITHUB_CLIENT_SECRET` + `BUILDHOST_GITHUB_LOGIN_ORGS`), `requireProject`'s `unauthorizedResponse` (`internal/auth/middleware.go`) `303`-redirects a **browser navigation** (`Accept: text/html`) with no token to the apex `/__signin?next=<full original URL>`, which runs the GitHub OAuth Authorization Code flow (`internal/auth/githublogin.go`). `handleSigninStart` sets a short-lived signed-state + CSRF nonce cookie and redirects to GitHub (`scope=read:org`); `handleSigninCallback` (the fixed `redirect_uri`) verifies the state + nonce, exchanges the code for a user token, and via `authorizedLogin` confirms the user is a member of an allowed org (`GET /user` + `GET /user/memberships/orgs/{org}`). On success it mints a buildhost-signed `bh_session` cookie (HMAC over `login|exp` with the shared `download-signing.key`; HttpOnly, SameSite=Lax, Secure-on-https, 12h) and redirects back to `next`. The cookie is set **domain-wide** (`Domain=<apex>`) because the GitHub OAuth app has one fixed callback (apex) but resources live on service subdomains -- so one login authenticates all of `sites/dl/static/...`. The `Authenticate` middleware verifies `bh_session` per request and sets `WithUser(login)`; `requireProject` treats a signed-in human as **read** authorization for private resources (ReadAccess and HiddenReadAccess) -- org membership is the gate. It never grants write (writes still require a `write` token). `safeNextURL` accepts only a same-site path or an apex/subdomain URL (no open redirect). The whole flow is apex-only (`/__signin`, `/__signin/callback`, `/__signout` via `HandleRaw`) since GitHub OAuth registers a single callback URL. When unconfigured, browsers fall back to the plain JSON `401` (programmatic clients and `/v2/` OCI are always unchanged). Public sites (`X-Public-Site: true`) are still served with no auth at all -- this only affects genuinely-gated reads. **Setup**: create a GitHub OAuth App with callback `https://{apex}/__signin/callback`, set the client id/secret env vars, and list the allowed orgs in `BUILDHOST_GITHUB_LOGIN_ORGS`.
