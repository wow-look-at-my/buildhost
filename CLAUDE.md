...
- `internal/npm/` - npm registry endpoint on `npm.{domain}/@buildhost/{project}`. Tarball URLs point to static. Self-registering via auth.OnReady(). The main domain also 301-redirects `{domain}/npm` and `{domain}/npm/...` to the `npm.{domain}` subdomain (preserving the scoped-package `%2f`), so the documented `npm install --registry {domain}/npm` works without the caller knowing about subdomain routing -- npm follows the redirect on packument requests.
