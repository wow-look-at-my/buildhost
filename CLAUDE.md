...
- **Browser sign-in via Cloudflare Access**: a browser hitting a private resource (e.g. a private static-site PR preview) used to get a raw JSON `401` with no way to authenticate. When Cloudflare Access is configured (`BUILDHOST_CF_ACCESS_TEAM_DOMAIN` + `BUILDHOST_CF_ACCESS_AUD`), `requireProject`'s `unauthorizedResponse` (`internal/auth/middleware.go`) now `303`-redirects a **browser navigation** (`Accept: text/html`) with no token to `/__access?next=<original-url>` -- a single path the operator protects with a Cloudflare Access self-hosted application. Cloudflare runs its hosted login there and forwards the request to buildhost with a `Cf-Access-Jwt-Assertion`; `handleCFAccessCallback` (`internal/auth/cfaccess.go`, registered on the apex + every service subdomain like `llms.txt`) verifies that JWT (`CFAccessVerifier`: signature against `<team>/cdn-cgi/access/certs`, `iss == team domain`, `aud == AUD tag`, expiry), then mints a buildhost-signed session cookie `bh_cfaccess` (HMAC over `email|exp` with the shared `download-signing.key`; HttpOnly, SameSite=Lax, Secure-on-https, 12h) and redirects back to `next`. The `Authenticate` middleware verifies that cookie on each request and sets `WithCFAccess(email)`; `requireProject` treats a Cloudflare-Access identity as **read** authorization for private resources (ReadAccess and HiddenReadAccess) -- the Access policy is the authorization gate. It never grants write. `safeNext` rejects absolute/scheme-relative `next` (no open redirect). This is buildhost-driven (not edge Access on the whole host) because `sites.{domain}` mixes public and private branches per-row (`is_public`); gating the whole subdomain at the edge would break the anonymous public previews, so only `/__access` is Access-protected and buildhost selectively redirects to it. **Setup**: create a Cloudflare Access self-hosted app covering `/__access` on the service hosts (e.g. `*.{domain}/__access` and `{domain}/__access`), copy its AUD tag, and set the two env vars. When unconfigured, browsers fall back to the plain JSON `401` (programmatic clients and `/v2/` OCI are always unchanged). Public sites (`X-Public-Site: true`) are still served with no auth at all -- this only affects genuinely-gated reads.
