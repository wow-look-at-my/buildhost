...
- **Browser sign-in on 401**: `requireProject`'s `unauthorizedResponse` (`internal/auth/middleware.go`) tailors the `401` by client. Programmatic clients (no `text/html` in `Accept`) get the unchanged bare JSON `{"error":"authentication required"}` with **no** challenge; `/v2/` (OCI) keeps its registry error envelope + `WWW-Authenticate: Basic`. A **browser navigation** (`Accept: text/html`) now also gets `WWW-Authenticate: Basic realm="buildhost"` -- which makes the browser show its native sign-in dialog -- plus a small HTML page (shown if the dialog is dismissed). Basic auth resolves the **password** to a token (`extractBasicAuth`), so a user pastes a `read`-scoped token as the password to view a private static site / PR preview in a browser. Without the challenge a browser just rendered raw JSON 401 with no way to authenticate (the original "no way to sign in" dead end). The HTML branch relaxes the global `default-src 'none'` CSP to `style-src 'unsafe-inline'` for that one response (no scripts, no external resources). This is orthogonal to public-site serving: a site published `X-Public-Site: true` is still served with no auth at all; this only affects genuinely-gated reads.
