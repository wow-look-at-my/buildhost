...
- Apex `latest` (a download with no `?v=` and no `?branch=` -- e.g. `dl/{project}/latest/{os}/{arch}`) resolves to the newest published release on the project's **default branch** (`projects.default_branch`), so a push to a feature branch never hijacks `latest`. The default branch is recorded from the create-release `default_branch` field (the publish composite actions derive it from `github.event.repository.default_branch`). When unset (legacy projects, non-GHA publishers) or when the default branch has no published release yet, resolution falls back to the newest release across all branches -- the historical behavior. This is centralized in `db.GetLatestRelease`, so every consumer (dl, brew, apt, web, the OCI `latest` tag, the npm `latest` dist-tag) is consistent. Per-branch downloads (`?branch=`) are unaffected.
