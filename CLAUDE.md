...
- Temporary download links: a private artifact can be shared without a project token via a short-lived, **artifact-bound, HMAC-signed** URL -- `static.{domain}/file?...&token=bhdl_...`. Mint with `POST /api/v1/projects/{project}/download-links` (REST, requires a `share`-scoped token authorized for the project) or `POST /api/projects/{name}/download-links` (admin dashboard, trusted behind its reverse proxy -- the SPA's "temp link" button on the release page calls it). The signature binds `(project, version, os, arch, fmt, debug)` + expiry (default 1h, max 24h), so a leaked link exposes only that one file until it expires; it is stateless (no DB row, not individually revocable -- rotate `download-signing.key` to invalidate all). The link points at `static` directly (no `dl` hop) with the version already resolved, since the binding needs an exact version.
