...
- **Browser sign-in (session cookie + login page)**: a browser hitting a private resource (e.g. a private static-site PR preview) used to get a raw JSON `401` with no `WWW-Authenticate` and no login UI -- a dead end. buildhost now serves its own sign-in flow (`internal/auth/login.go`, `session.go`). `requireProject`'s `unauthorizedResponse` (`internal/auth/middleware.go`) branches by client: programmatic callers (no `text/html` in `Accept`) keep the unchanged bare JSON `401`; `/v2/` (OCI) keeps its registry error envelope + `WWW-Authenticate: Basic`; a **browser navigation** with **no token** is `303`-redirected to `/__auth/login?next=<original-url>`. The login page (`GET/POST /__auth/login`, plus `GET /__auth/logout`, registered on the apex and every service subdomain like `llms.txt`) takes a buildhost token, validates it (`validateLoginToken` -- static token or OIDC JWT), and stores it in an **HttpOnly, SameSite=Lax, Secure-on-https session cookie** `bh_session` (12h). `ExtractToken` reads that cookie as its **lowest-priority** source (an explicit `Authorization`/Basic/`?token=` still wins), so the rest of the token-auth model is unchanged; SameSite=Lax keeps the cookie off cross-site writes (CSRF posture intact). To avoid a redirect loop, a browser that **already** carries a token but is still unauthorized (e.g. cookie token lacks `read`, or `forbiddenResponse`'s 403 for a wrong-project token) gets an HTML **access-denied** page (links to sign in as a different token / sign out), never another redirect. `safeNext` rejects absolute/scheme-relative `next` targets (no open redirect). Public sites (`X-Public-Site: true`) are still served with no auth at all -- this only affects genuinely-gated reads. Note: this is buildhost-native because `sites.{domain}` mixes public and private branches per-row (`is_public`), so a blanket Cloudflare Access at the edge can't distinguish them without breaking public previews; Cloudflare Access remains the model only for the all-private admin dashboard.
