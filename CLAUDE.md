...
- `internal/repackage/` - On-demand repackaging and stripping (tar.gz, tar.xz, tar.zst, zip, deb, brew, npm, oci). Self-registering via init(); Generator uses registry. Orchestrator just publishes releases. `cacerts/ca-certificates.crt` is an embedded (`//go:embed`) public CA bundle baked into the synthesized OCI essentials layer; refresh it from `https://curl.se/ca/cacert.pem` (see `cacerts/README.md`).
