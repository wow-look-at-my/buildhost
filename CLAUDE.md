...
- `internal/npm/` - npm registry endpoint on `npm.{domain}/@buildhost/{project}`. Tarball URLs point to static. Self-registering via auth.OnReady(). The path-based `npm install --registry {domain}/npm` works via the main-domain service redirect (below); npm follows the 301 on packument requests, preserving the scoped-package `%2f`.
...
- `internal/auth/` - Token auth, OIDC JWT verification, centralized project-auth middleware (requireProject), route registry (Handle/HandleRaw/HandleHandler/ServiceRoute/ServiceRedirect) backed by `github.com/wow-look-at-my/router`, RouteInfo interface. Registering any service also auto-registers a main-domain path redirect `{domain}/<service>/... -> <service>.{domain}/...` (preserving the raw `%2f` and query string), so path-style URLs resolve without addressing the subdomain
