...
- **OIDC audience**: Auto-provisioning does **not** require the token's `aud` to name this server (clients should not have to echo the buildhost URL back to it). Trust is established by the signed issuer (`BUILDHOST_OIDC_ISSUERS`), the org allowlist (`BUILDHOST_OIDC_ORGS`), the event allowlist (`BUILDHOST_OIDC_EVENTS`), and the `sub`->project mapping -- a repo's token only ever authorizes its own project namespace, so a token minted for another audience cannot reach a different project. Because the audience no longer backstops the org allowlist, scope `BUILDHOST_OIDC_ORGS` to trusted orgs (never `*` in production). A configured policy `audience` is still enforced for that policy (the policy path is unchanged).
