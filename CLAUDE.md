...
- **Sites security headers**: Served site files drop the app's strict `Content-Security-Policy: default-src 'none'` and `X-Frame-Options: DENY` that the global middleware sets (`internal/sites/serve.go` `relaxSiteSecurityHeaders`). Hosted sites are third-party static content on the dedicated `sites.{domain}` subdomain (isolated from the app/admin origins) and must be able to load their own and external assets, like any static host. They also set `Access-Control-Allow-Origin: *` for non-credentialed cross-origin asset reads. `X-Content-Type-Options: nosniff` is kept.
